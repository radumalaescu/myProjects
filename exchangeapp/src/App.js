  ...
  const updateBallanceError = (firstValue, secondValue, firstBallance, secondBallance) => {
    if (+firstValue <= 0 || +secondValue <= 0) {
      ...
      if (firstValue === ronGbpValue) {
        return updateBallance(firstValue, secondValue, firstBallance, secondBallance, setRonBallance, setGbpBallance)
      }
      if (firstValue === eurUsdValue) {
        return updateBallance(firstValue, secondValue, firstBallance, secondBallance, setEurBallance, setUsdBallance)
      }
      if (firstValue === eurGbpValue) {
        return updateBallance(firstValue, secondValue, firstBallance, secondBallance, setEurBallance, setGbpBallance)
      }
      if (firstValue === usdGbpValue) {
        return updateBallance(firstValue, secondValue, firstBallance, secondBallance, setUsdBallance, setGbpBallance)
      }
    }
  }
        ...
        <Switch>
          <Route path='/exchange'>
            <Exchange
              ...
              changeValueUsdRon={(event) => changeValue(event, setUsdRonValue, setRonUsdValue, 4.06)}
              changeValueRonGbp={(event) => changeValue(event, setRonGbpValue, setGbpRonValue, 0.17)}
              changeValueGbpRon={(event) => changeValue(event, setGbpRonValue, setRonGbpValue, 5.73)}
              changeValueEurUsd={(event) => changeValue(event, setEurUsdValue, setUsdEurValue, 1.22)}
              changeValueUsdEur={(event) => changeValue(event, setUsdEurValue, setEurUsdValue, 0.82)}
              changeValueEurGbp={(event) => changeValue(event, setEurGbpValue, setGbpEurValue, 0.86)}
              changeValueGbpEur={(event) => changeValue(event, setGbpEurValue, setEurGbpValue, 1.16)}
              changeValueUsdGbp={(event) => changeValue(event, setUsdGbpValue, setGbpUsdValue, 0.71)}
              changeValueGbpUsd={(event) => changeValue(event, setGbpUsdValue, setUsdGbpValue, 1.41)}
              ...
              clickDisableRonGbp={() => clickDisable(setRonGbpValue, setGbpRonValue)}
              clickDisableEurUsd={() => clickDisable(setEurUsdValue, setUsdEurValue)}
              clickDisableEurGbp={() => clickDisable(setEurGbpValue, setGbpEurValue)}
              clickDisableUsdGbp={() => clickDisable(setUsdGbpValue, setGbpUsdValue)}
              ...
              updateBallanceErrorRonGbp={() => updateBallanceError(ronGbpValue, gbpRonValue, ronBallance, gbpBallance)}
              updateBallanceErrorEurUsd={() => updateBallanceError(eurUsdValue, usdEurValue, eurBallance, usdBallance)}
              updateBallanceErrorEurGbp={() => updateBallanceError(eurGbpValue, gbpEurValue, eurBallance, gbpBallance)}
              updateBallanceErrorUsdGbp={() => updateBallanceError(usdGbpValue, gbpUsdValue, usdBallance, gbpBallance)}
